...
export const FieldRenderer: React.FC<FieldRendererProps> = ({
  ...
  const baseInputClass = `w-full rounded-lg border px-3 py-2 text-xs font-medium transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-blue-500/20 shadow-2xs ${
    error
      ? 'border-rose-500 focus:border-rose-500 focus:ring-rose-500/20 bg-rose-50/10'
      : 'border-slate-300 dark:border-slate-700 bg-background text-foreground focus:border-blue-600'
  } ${
    !isEditable
      ? 'cursor-not-allowed opacity-60 bg-slate-100 dark:bg-slate-800 text-muted-foreground'
      : 'hover:border-blue-400'
  }`;

  return (
    <div className="group rounded-xl border border-slate-200 dark:border-slate-800 bg-slate-50/40 dark:bg-slate-900/40 p-3.5 space-y-2 transition-all duration-200 hover:border-blue-400/60 hover:bg-slate-50/80 dark:hover:bg-slate-900/80 hover:shadow-2xs focus-within:border-blue-600 focus-within:ring-2 focus-within:ring-blue-500/20 focus-within:bg-card">
      {/* Label and Tooltip Header */}
      <div className="flex items-center justify-between">
        <label
          htmlFor={field.internalName}
          className="flex items-center gap-1.5 text-xs font-bold text-foreground group-hover:text-blue-600 transition-colors"
        >
        ...
        {field.excel?.columnName && (
          <span className="text-[10px] font-mono font-medium text-muted-foreground bg-muted border border-border px-1.5 py-0.5 rounded">
      ...
      {/* Error Message */}
      {error && (
        <div className="flex items-center gap-1 text-[11px] font-medium text-rose-500 mt-1">
