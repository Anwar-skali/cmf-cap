...
export function MappingPreviewComponent({ mappingResult, mapping, onMappingChange }: MappingPreviewProps) {
  ...
  const fallbackReason = mappingResult.fallbackReason ?? null;
  ...
  return (
    <div className="space-y-4">
      ...
      >
        <div className="flex items-center gap-3">
          {ollamaActive ? (
            <>
              ...
              <span>
                <span className="font-bold">✔ AI Mapping Active</span> — Semantic mapping generated by local{' '}
                <code className="font-mono text-xs font-bold bg-violet-500/10 px-1.5 py-0.5 rounded">{model}</code>{' '}
                model via RAG. All column matches are LLM-powered.
              </span>
            </>
          ) : ollamaReachable ? (
            <>
              ...
              <span>
                <span className="font-bold">⚠ Ollama Connected — Fuzzy Fallback Mode</span>
                {fallbackReason && (
                  <span className="block text-xs text-blue-700 mt-0.5 font-normal">
            ...
            <>
              ...
              <span>
                <span className="font-bold">⚠ Local Ollama Not Reachable</span> — Ollama service unavailable at{' '}
                <code className="font-mono text-xs bg-amber-500/10 px-1 py-0.5 rounded">http://localhost:11434</code>.
                Using deterministic fuzzy matching. Run{' '}
                <code className="font-mono text-xs bg-amber-500/10 px-1 py-0.5 rounded">ollama serve</code> to enable AI.
                {fallbackReason && (
                  <span className="block text-xs text-amber-700 mt-0.5 font-normal">
                    Reason: {fallbackReason}
                  </span>
                )}
              </span>
        ...
        {executionTimes && (
          <div className="flex items-center gap-2 text-xs shrink-0 self-end sm:self-auto font-mono text-muted-foreground">
            <span>
              Excel: <strong className="text-foreground">{executionTimes.excelReadMs ?? executionTimes.headerResolutionMs ?? 0}ms</strong> |{' '}
              Prompt: <strong className="text-foreground">{executionTimes.promptBuildMs ?? executionTimes.templateLoadingMs ?? 0}ms</strong> |{' '}
              AI: <strong className="text-violet-700">{executionTimes.ollamaResponseTimeMs ? `${(executionTimes.ollamaResponseTimeMs / 1000).toFixed(1)}s` : '0s'}</strong> |{' '}
              Parse: <strong className="text-foreground">{executionTimes.jsonParseMs ?? 0}ms</strong> |{' '}
              Total: <strong className="text-primary font-bold">{(executionTimes.totalMappingMs ? executionTimes.totalMappingMs / 1000 : 0).toFixed(2)}s</strong>
            </span>
          </div>
        )}
      </div>
